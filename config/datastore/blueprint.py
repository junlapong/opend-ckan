...
def dump(resource_id):
    ...
    if data[u'format'] == u'csv':
        response.headers[u'content-type'] = b'text/csv; charset=utf-8'
        response.headers['Content-disposition'] = (b'attachment; filename="{resource_id}.csv"'.format(resource_id=encode_rfc2231(resource_id)))
    elif data[u'format'] == u'tsv':
        response.headers[u'content-type'] = b'text/tab-separated-values; charset=utf-8'
        response.headers['Content-disposition'] = (b'attachment; filename="{resource_id}.tsv"'.format(resource_id=encode_rfc2231(resource_id)))
    elif data[u'format'] == u'json':
        response.headers[u'content-type'] = b'application/json; charset=utf-8'
        response.headers['Content-disposition'] = (b'attachment; filename="{resource_id}.json"'.format(resource_id=encode_rfc2231(resource_id)))
    elif data[u'format'] == u'xml':
        response.headers[u'content-type'] = b'text/xml; charset=utf-8'
        response.headers['Content-disposition'] = (b'attachment; filename="{resource_id}.xml"'.format(resource_id=encode_rfc2231(resource_id)))
